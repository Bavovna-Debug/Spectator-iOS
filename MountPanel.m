...
@property (strong, nonatomic) DiskUsageGraph  *diskUsageGraph;

@end

@implementation MountPanel
...
- (id)initWithMount:(SRVMountRecord *)mount
{
    ...
    self.diskUsageGraph = [[DiskUsageGraph alloc] initWithFrame:graphFrame
                                                          mount:mount];
...
- (void)refresh
{
    ...
    if ([self showBytes]) {
        text = [NSString stringWithFormat:@"%@  (%0.0f)",
                [self formattedSize:totalSize units:FormatSizeUnitsAuto], totalSize];
        [self.totalValue setText:text];
        
        text = [NSString stringWithFormat:@"%@  (%0.0f)",
                [self formattedSize:usedSize units:FormatSizeUnitsAuto], usedSize];
        [self.usedValue setText:text];
        
        text = [NSString stringWithFormat:@"%@  (%0.0f)",
                [self formattedSize:availableSize units:FormatSizeUnitsAuto], availableSize];
        [self.availableValue setText:text];
        
        text = [NSString stringWithFormat:@"%@  (%0.0f)",
                [self formattedSize:reservedSize units:FormatSizeUnitsAuto], reservedSize];
        [self.reservedValue setText:text];
    } else {
        [self.totalValue setText:[self formattedSize:totalSize units:FormatSizeUnitsAuto]];
        [self.usedValue setText:[self formattedSize:usedSize units:FormatSizeUnitsAuto]];
        [self.availableValue setText:[self formattedSize:availableSize units:FormatSizeUnitsAuto]];
        [self.reservedValue setText:[self formattedSize:reservedSize units:FormatSizeUnitsAuto]];
    }
